...
function romanNumeralToInt (roman) {
    ...
    for (let i=0;i<roman.length;i++) {
        if (romanToIntMapping[roman[i]] < romanToIntMapping[roman[i+1]]) sum -= romanToIntMapping[roman[i]]
        else sum += romanToIntMapping[roman[i]]
    }
    return sum
}

console.log(`roman to integer: ${romanNumeralToInt(s)}`)
console.log(`roman to integer: ${romanNumeralToInt(t)}`)
console.log(`roman to integer: ${romanNumeralToInt(u)}`)